...
if "df" in st.session_state and st.session_state.df is not None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Tabela Completa", "Relatório Registro Entradas", "Relatório PisCofins", "Relatório Analítico Pis/Cofins"])
    ...
    with tab1:
            ...
            col1.metric("Total BC ICMS", f"{df['BC_ICMS'].sum():.2f}", border=True, width='content')
            col2.metric("Total Vlr ICMS", f"{df['vlr_ICMS'].sum():.2f}", border=True, width='content')
            col3.metric("Total BC Pis", f"{df['BC_PIS_Calc'].sum():.2f}", border=True, width='content')
            col4.metric("Total BC Cofins", f"{df['BC_Cofins_Calc'].sum():.2f}", border=True, width='content')
